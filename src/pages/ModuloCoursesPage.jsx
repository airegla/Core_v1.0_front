// BookOS Core - ModuloCourses.jsx
// ruta: Core/frontend/src/pages/ModuloCourses.jsx
// descripcion: pagina del modulo "Cursos" (adoptado de la tabla legacy "courses" por el
//   nucleo de desarrollo): listado y alta contra la API del Core.
// estilos: los del core (card/btn/input-os y el patron de colores del tema por variables CSS); para reemplazarlos, crear src/styles/modulo-courses.css y regenerar (o importarlo a mano).
...
export default function ModuloCourses() {
  ...
  const cargar = useCallback(() => {
    setCargando(true);
    axiosClient.get('/courses', { params: { limite: 300 } })
  ...
  return (
    <div>
      <DebugTag nombre="ModuloCourses" />
      <h2 className="text-lg font-semibold mb-1">Cursos</h2>
      ...
      <form className="card p-4 mb-4 flex flex-wrap items-end gap-3" onSubmit={guardar}>
        <label className="block text-sm">
          <span className="block text-xs uppercase tracking-widest text-muted mb-1">code</span>
          <input className="input-os" value={form.code} onChange={cambio('code')} />
        </label>
        <label className="block text-sm">
        ...
        <label className="block text-sm">
          <span className="block text-xs uppercase tracking-widest text-muted mb-1">description</span>
          <input className="input-os" value={form.description} onChange={cambio('description')} />
        </label>
        <label className="block text-sm">
          ...
          <tbody>
            {filas.map((f, i) => (
              <tr key={f.id || i} className="border-t" style={{ borderColor: 'var(--border, #333)' }}>
                <td className="py-1">{String(f.code ?? '')}</td>
                <td className="py-1">{String(f.name ?? '')}</td>
                <td className="py-1">{String(f.description ?? '')}</td>
